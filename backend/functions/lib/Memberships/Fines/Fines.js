...
app.get('/membershipFines', (req, res) => {
    (async () => {
        try {
            ...
            const response = [];
            await fines.get().then((querySnapshot) => {
                const docs = querySnapshot.docs;
                for (const doc of docs) {
                    const selectedItem = {
                        fineId: doc.id,
                    ...
                    if (req.query.fineState === "paid" && selectedItem.isPaid)
                        response.push(selectedItem);
                    else if (req.query.fineState === "noPaid" && !selectedItem.isPaid)
                        response.push(selectedItem);
                    else if (req.query.fineState === "all")
                        response.push(selectedItem);
                }
                return response;
            });
...
app.get('/teamFines', (req, res) => {
    (async () => {
        try {
            ...
            await teamMemberships.get().then((querySnapshot) => {
                const docs = querySnapshot.docs;
                for (const doc of docs) {
                    const info = {
            ...
            for (const member of members) {
                ...
                await membership.get().then((finesSnapshot) => {
        ...
        try {
            ...
            await fines.get().then((querySnapshot) => {
                ...
                for (const doc of docs) {
                    ...
                    if (doc.data().isPaid)
                        precios.paid += doc.data().money;
                    else if (!doc.data().isPaid)
                        precios.pending += doc.data().money;
                }
                return precios;
            });
            ...
            for (const member of members) {
                ...
                await membership.get().then((finesSnapshot) => {
                    ...
                    for (const fine of fines) {
                        ...
                        if (fine.data().isPaid)
                            precios.paid += fine.data().money;
                        else if (!fine.data().isPaid)
                            precios.pending += fine.data().money;
                    }
                });
            }
            return res.status(200).send(precios);
